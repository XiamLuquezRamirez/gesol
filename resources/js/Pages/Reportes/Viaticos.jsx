...
/** Fila expandible de una comisión: al abrir muestra viajeros, rubros y comprobantes. */
        ...
        <div className="rounded-lg border border-slate-100">
            ...
            >
                <div className="min-w-0">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-slate-500">{comision.viajeros.length} viajero(s)</span>
                    </div>
            ...
            {abierta && (
                <div className="border-t border-slate-100 divide-y divide-slate-50">
                    {comision.viajeros.map((v, i) => (
                        <div key={i} className="px-4 py-3">
                            <div className="flex items-center justify-between gap-3">
                                <p className="text-sm font-medium text-slate-700">{v.nombre}</p>
                                <span className="text-sm font-semibold text-slate-700">{formatearMoneda(v.total)}</span>
                            </div>
                            ...
                            {/* Comprobantes de pago del viajero (a quien se asignó ese comprobante) */}
                            <div className="mt-2">
                                <p className="text-xs font-medium text-slate-500 mb-1">
                                    Comprobantes de pago ({v.comprobantes.length})
                                </p>
...
export default function ReporteViaticos({ filtros, reporte }) {
    return (
        <VistaInforme
            titulo="Viáticos liquidados"
            subtitulo="Comisiones del rango (por fecha de comisión). Abre cada una para ver viajeros, rubros y comprobantes."
            ruta="reportes.viaticos"
            exportable
            filtros={filtros}
        >
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 mb-2">
                ...
                <Tarjeta label="Viajeros" valor={reporte.num_viajeros} />
            </div>

            {reporte.comisiones.length === 0 ? (
                <Vacio>No hay comisiones en este rango.</Vacio>
            ) : (
                <div className="space-y-2">
                    {reporte.comisiones.map((c) => (
                        <FilaComision key={c.solicitud_id} comision={c} />
                    ))}
                    <div className="flex items-center justify-between rounded-lg bg-slate-800 px-4 py-3 text-white">
                        <span className="text-sm font-medium">Total de todas las comisiones</span>
